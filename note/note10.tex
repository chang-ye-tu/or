...
\usepackage{amssymb, xcolor, amsmath, soul, multirow, caption, graphicx, comment, amsthm, adjustbox, dsfont}
...
%\newcommand{\x}{\bm{x}}  
\newcommand{\X}{\mathbf{X}}%{\bm{X}}
\newcommand{\z}{\mathbf{z}}%{\bm{z}}
\newcommand{\Z}{\mathbf{Z}}%{\bm{Z}}
\newcommand{\s}{\mathbf{s}}%{\bm{s}}                    
%\newcommand{\y}{\bm{y}}            
\newcommand{\Y}{\mathbf{Y}}%{\bm{Y}}  	   
\newcommand{\W}{\mathbf{W}}%{\bm{W}}   	       
\newcommand{\bb}{\mathbf{b}}%{\bm{b}}        	          
\newcommand{\btheta}{\theta}%{\bm{\theta}}
%\newcommand{\f}{\bm{f}} 
\newcommand{\Hp}{\Theta}%{\bm{\Theta}} 
\newcommand{\g}{\mathbf{g}}%{\bm{g}}
\newcommand{\G}{\mathbf{G}}%{\bm{G}}
\newcommand{\V}{\mathbf{V}}%{\bm{V}}
...
% Maps and functions
\newcommand{\gen}{\mathbf{g}}%{\bm{g}}       	                % generator
...
\begin{document}
\title{\texorpdfstring{\vspace{15mm} 作業研究\\ 10. Neural Network (NN)}{作業研究\\ 10. Neural Network (NN)}} 
\author{}
\date{}
\maketitle
...
\begin{align}
  ...
  \df{\vx^{(l)}}{\xi^{(l)}} &= \mathbf{S}^{(l)} \equiv {\rm diag}[\sigma'(\xi^{(l)}_1), \cdots, \sigma'(\xi^{(l)}_{H_l})],
\end{align}
...
\begin{align}
  \df{\Pi}{\xi^{(l)}} &= \label{eqn:dpidxi1} \df{\Pi}{\vx^{(L+1)}} \cdot \mathbf{S}^{(L+1)} \cdot \vW^{(L+1)} \cdots \mathbf{S}^{(l+1)} \cdot \vW^{(l+1)} \cdot \mathbf{S}^{(l)}. 
\end{align}
Taking the transpose, and recognizing that $\Sigma^{(l)}$ is diagonal and therefore symmetric, we finally arrive at
\begin{align}
  \df{\Pi}{\xi^{(l)}} &= \label{eqn:dpidxifinal} \mathbf{S}^{(l)} \vW^{(l+1)T} \mathbf{S}^{(l+1)} \cdots   \vW^{(L+1)T} \mathbf{S}^{(L+1)} [-2 (\vy-\vx^{(L+1)})]. 
\end{align}
...
\begin{align}\label{eqn:bpr1}
  \df{\Pi}{\vW^{(l)}} &= \df{\Pi}{\xi^{(l)}} \cdot \df{ \xi^{(l)} }{\mathbf{W}^{(l)}}  = \df{\Pi}{\xi^{(l)}} \otimes \vx^{(l-1)}, 
\end{align}
