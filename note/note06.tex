...
\begin{document}
\title{\texorpdfstring{\vspace{15mm} 作業研究\\ 06. Convex Optimization Problems}{作業研究\\ 06. Convex Optimization Problems}} 
\author{}
\date{}
\maketitle
\newpage
...
\begin{itemize}
  ...
  \item $\ds a_i^\top x\sim\mathsf{N}(\overline{a}_i^\top x, x^\top\Sigma_i\,x)$, so
    \begin{align*}
      \prb\{a_i^\top\leqslant b_i\} = \Phi\Bigg(\frac{b_i - \overline{a}_i^\top x}{\|\Sigma_i^\frac{1}{2} x\|_2}\Bigg)
    \end{align*}
    where $\ds\Phi(u) = \frac{1}{\sqrt{2\pi}}\int_{-\infty}^u e^{-\frac{t^2}{2}}\,\text{d}t\;$ (cdf of $\mathsf{N}(0, 1)$)
  \item $\ds\prb\{a_i^\top\leqslant b_i\}\geqslant\eta$ can be expressed as $\overline{a}_i^\top x + \Phi^{-1}(\eta)\,\big\|\,\Sigma_i^\frac{1}{2} x\,\big\|_2\leqslant b_i$
...
\begin{itemize}
  \item risk-return trade-off: minimize $\ds(-\overline{p}^\top x, x^\top\Sigma\,x)$, subject to \\$\mathbf{1}^\top x = 1$, $x\preccurlyeq 0$
